...
for (m in 1:length(naics)){
  ...
  write(stargazer(pols1_amen_r_coef, pols1_r_coef,
  ...
  pols_namen_nc_real <- lm(master$births_ratio ~ master$ptax_diff + master$inctax_diff + master$capgntax_diff
                           ...
                           subset = master$births_sub > 0 & master$births_nbr > 0)
  stprid_c_vcov <- cluster.vcov(pols_namen_nc_real, master$stpr_id[master$births_sub > 0 & master$births_nbr > 0])
  pols_namen_nc_r_coef <- coeftest(pols_namen_nc_real, vcov = stprid_c_vcov) # results
  
  pols_amen_nc_real <- lm(master$births_ratio ~ master$ptax_diff + master$inctax_diff + master$capgntax_diff
                          + master$salestax_diff  + master$corptax_diff  + master$wctax_diff  + master$uitax_diff
                          + master$educ_pc_L1_diff + master$hwy_pc_L1_diff + master$welfare_pc_L1_diff
                          + master$JAN.TEMP.Z_diff + master$JAN.SUN.Z_diff + master$JUL.TEMP.Z_diff + master$JUL.HUM.Z_diff
                          + master$TOPOG.Z_diff + master$WATER.AR.Z_diff, 
                          subset = master$births_sub > 0 & master$births_nbr > 0)
  stprid_c_vcov <- cluster.vcov(pols_amen_nc_real , master$stpr_id[master$births_sub > 0 & master$births_nbr > 0])
  pols_amen_nc_r_coef <- coeftest(pols_amen_nc_real , vcov = stprid_c_vcov) # results
  
  pols_namen_c_r <- lm(master$births_ratio ~ master$ptax_diff + master$inctax_diff + master$capgntax_diff
                       + master$salestax_diff  + master$corptax_diff  + master$wctax_diff  + master$uitax_diff
                       + master$educ_pc_L1_diff + master$hwy_pc_L1_diff + master$welfare_pc_L1_diff
                       + master$hsplus.cont_diff + master$realfuelpr.cont_diff + master$unionmem.cont_diff
                       + master$popdensity.cont_diff + master$pctmanuf.cont_diff, 
                       subset = master$births_sub > 0 & master$births_nbr > 0)
  stprid_c_vcov <- cluster.vcov(pols_namen_c_r,  master$stpr_id[master$births_sub > 0 & master$births_nbr > 0])
  pols_namen_c_r_coef <- coeftest(pols_namen_c_r, vcov = stprid_c_vcov)
  
  linearHypothesis(pols_namen_c_r, c("master$ptax_diff + master$inctax_diff + master$capgntax_diff
                    + master$salestax_diff  + master$corptax_diff  + master$wctax_diff  + master$uitax_diff = 0"), vcov = stprid_c_vcov)
  linearHypothesis(pols_namen_c_r, c("master$capgntax_diff + master$salestax_diff  + master$corptax_diff  + master$wctax_diff  + master$uitax_diff = 0"), vcov = stprid_c_vcov)
  
  
  pols_amen_c_r <- lm(master$births_ratio ~ master$ptax_diff + master$inctax_diff + master$capgntax_diff
                      + master$salestax_diff  + master$corptax_diff  + master$wctax_diff  + master$uitax_diff
                      + master$educ_pc_L1_diff + master$hwy_pc_L1_diff + master$welfare_pc_L1_diff
                      + master$hsplus.cont_diff + master$realfuelpr.cont_diff + master$unionmem.cont_diff
                      + master$popdensity.cont_diff + master$pctmanuf.cont_diff + master$JAN.TEMP.Z_diff
                      + master$JAN.SUN.Z_diff + master$JUL.TEMP.Z_diff + master$JUL.HUM.Z_diff
                      + master$TOPOG.Z_diff + master$WATER.AR.Z_diff, 
                      subset = master$births_sub > 0 & master$births_nbr > 0)
  stprid_c_vcov <- cluster.vcov(pols_amen_c_r, master$stpr_id[master$births_sub > 0 & master$births_nbr > 0])
  ...
  simple.pop.2s<- lm(master$births_ratio ~ master$ptax_diff + master$inctax_diff + master$capgntax_diff
                     ...
                     + master$educ_pc_L1_diff + master$hwy_pc_L1_diff + master$welfare_pc_L1_diff
                     + master$hsplus.cont_diff + master$realfuelpr.cont_diff + master$unionmem.cont_diff
                     + master$popdensity.cont_diff + master$pctmanuf.cont_diff + master$pop.pred, 
                     subset = master$births_sub > 0 & master$births_nbr > 0)
  ...
  pols_amen_fe  <- felm(master$births_ratio ~ master$ptax_diff + master$inctax_diff + master$capgntax_diff
                        ...
                        + master$popdensity.cont_diff + master$pctmanuf.cont_diff  + master$JAN.TEMP.Z_diff
                        + master$JAN.SUN.Z_diff + master$JUL.TEMP.Z_diff + master$JUL.HUM.Z_diff
                        + master$TOPOG.Z_diff + master$WATER.AR.Z_diff  | master$stpr_fe | 0 |0)
  
  pols_fe <- felm(master$births_ratio ~ master$ptax_diff + master$inctax_diff + master$capgntax_diff
                  + master$salestax_diff  + master$corptax_diff  + master$wctax_diff  + master$uitax_diff
                  + master$educ_pc_L1_diff + master$hwy_pc_L1_diff + master$welfare_pc_L1_diff
                  + master$hsplus.cont_diff + master$realfuelpr.cont_diff + master$unionmem.cont_diff
                  + master$popdensity.cont_diff + master$pctmanuf.cont_diff | master$stpr_fe | 0 | 0)
  
  # write tables
  write(stargazer(pols_amen_c_r_coef, pols_namen_c_r_coef, pols_amen_nc_r_coef, pols_namen_nc_r_coef, pols_amen_fe, pols_fe, simple.pop.2s_coef,
                  dep.var.labels = c("births ratio"), model.names = FALSE,
                  ...
                  column.labels = c("OLS","OLS","OLS","OLS","FE", "FE","IV"),
                  no.space = TRUE, title = paste("Pseudo-RD Models for ", naics_names[m], "Firm Births", sep = " ")),
  ...
  dl_amen_c_r <- lm(dl_master$births_ratio ~ dl_master$ptax_diff + dl_master$inctax_diff + dl_master$capgntax_diff
                    ...
                    + dl_master$hsplus.cont_diff + dl_master$realfuelpr.cont_diff + dl_master$unionmem.cont_diff
                    + dl_master$popdensity.cont_diff + dl_master$pctmanuf.cont_diff + dl_master$JAN.TEMP.Z_diff
  ...
  dl_namen_c_r <- lm(dl_master$births_ratio ~ dl_master$ptax_diff + dl_master$inctax_diff + dl_master$capgntax_diff
                     + dl_master$salestax_diff  + dl_master$corptax_diff  + dl_master$wctax_diff  + dl_master$uitax_diff
                     + dl_master$educ_pc_L1_diff + dl_master$hwy_pc_L1_diff + dl_master$welfare_pc_L1_diff
                     + dl_master$hsplus.cont_diff + dl_master$realfuelpr.cont_diff + dl_master$unionmem.cont_diff
                     + dl_master$popdensity.cont_diff + dl_master$pctmanuf.cont_diff)
  stprid_c_vcov <- cluster.vcov(dl_namen_c_r,  dl_master$stpr_id)
  dl_namen_c_r_coef <- coeftest(dl_namen_c_r, vcov = stprid_c_vcov)
  
  dl_amen_nc_r <- lm(dl_master$births_ratio ~ dl_master$ptax_diff + dl_master$inctax_diff + dl_master$capgntax_diff
                     + dl_master$salestax_diff  + dl_master$corptax_diff  + dl_master$wctax_diff  + dl_master$uitax_diff
                     + dl_master$educ_pc_L1_diff + dl_master$hwy_pc_L1_diff + dl_master$welfare_pc_L1_diff
                     + dl_master$JAN.TEMP.Z_diff + dl_master$JAN.SUN.Z_diff + dl_master$JUL.TEMP.Z_diff + dl_master$JUL.HUM.Z_diff
                     + dl_master$TOPOG.Z_diff + dl_master$WATER.AR.Z_diff)
  stprid_c_vcov <- cluster.vcov(dl_amen_nc_r, dl_master$stpr_id)
  dl_amen_nc_r_coef <- coeftest(dl_amen_nc_r, vcov = stprid_c_vcov)
  
  dl_namen_nc_r <- lm(dl_master$births_ratio ~ dl_master$ptax_diff + dl_master$inctax_diff + dl_master$capgntax_diff
                      + dl_master$salestax_diff  + dl_master$corptax_diff  + dl_master$wctax_diff  + dl_master$uitax_diff
                      + dl_master$educ_pc_L1_diff + dl_master$hwy_pc_L1_diff + dl_master$welfare_pc_L1_diff)
  ...
  dl.pop.2s<- lm(dl_master$births_ratio ~ dl_master$ptax_diff + dl_master$inctax_diff + dl_master$capgntax_diff
                 ...
                 + dl_master$educ_pc_L1_diff + dl_master$hwy_pc_L1_diff + dl_master$welfare_pc_L1_diff
                 + dl_master$hsplus.cont_diff + dl_master$realfuelpr.cont_diff + dl_master$unionmem.cont_diff
                 + dl_master$popdensity.cont_diff + dl_master$pctmanuf.cont_diff + dl_master$pop.pred)
  ...
  dl_amen_fe  <- felm(dl_master$births_ratio ~ dl_master$ptax_diff + dl_master$inctax_diff + dl_master$capgntax_diff
                      ...
                      + dl_master$popdensity.cont_diff + dl_master$pctmanuf.cont_diff  + dl_master$JAN.TEMP.Z_diff
                      + dl_master$JAN.SUN.Z_diff + dl_master$JUL.TEMP.Z_diff + dl_master$JUL.HUM.Z_diff
                      + dl_master$TOPOG.Z_diff + dl_master$WATER.AR.Z_diff  | dl_master$stpr_fe | 0 | 0)
  
  dl_fe <- felm(dl_master$births_ratio ~ dl_master$ptax_diff + dl_master$inctax_diff + dl_master$capgntax_diff
                + dl_master$salestax_diff  + dl_master$corptax_diff  + dl_master$wctax_diff  + dl_master$uitax_diff
                + dl_master$educ_pc_L1_diff + dl_master$hwy_pc_L1_diff + dl_master$welfare_pc_L1_diff
                + dl_master$hsplus.cont_diff + dl_master$realfuelpr.cont_diff + dl_master$unionmem.cont_diff
                + dl_master$popdensity.cont_diff + dl_master$pctmanuf.cont_diff | dl_master$stpr_fe | 0 | 0)
  
  write(stargazer(dl_amen_c_r_coef, dl_amen_nc_r_coef, dl_namen_c_r_coef,dl_namen_nc_r_coef, dl_amen_fe, dl_fe, dl.pop.2s_coef, 
                  dep.var.labels = c("births ratio"), model.names = FALSE,
                  covariate.labels = c("Property Tax Difference", "Income Tax Difference", "Capital Gains Tax Difference",
                                       "Sales Tax Difference", "Corp Tax Difference", "Workers Comp Tax Difference",
                                       "Unemp. Tax Difference", "Educ Spending Per Cap Diff", "Highway Spending Per Cap Diff",
                                       "Welfare Spending Per Cap Diff"),
                  omit = c("cont","Z"), omit.labels = c('controls', 'amenities'), omit.yes.no = c("Yes", "No"), omit.table.layout = "sn",
                  column.labels = c("OLS","OLS","OLS","OLS","FE","IV"),
                  no.space = TRUE, title = paste("Pseudo-RD Models for ", naics_names[m], "Firm Births using Donald and Lang (2007)", sep = " ")), 
  ...
  write(stargazer(pols_1999,pols_2000,pols_2001,pols_2002,pols_2003,pols_2004,pols_2005,pols_2006,pols_2007,pols_2008,pols_2009, 
        ...
        paste("~/papers/firm_entry/analysis/output/", naics[m], "year_eb_results.tex", sep = "_"))
